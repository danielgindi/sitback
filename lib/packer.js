...
const copyFileAsync = require('util').promisify(require('fs').copyFile);
...
const someAsync = require('../utils/someAsync');
const everyAsync = require('../utils/everyAsync');

const GitUtil = require('../utils/git');
const GitDiffStatus = GitUtil.GitDiffStatus;
const MsbuildUtil = require('../utils/msbuild');
 ...
 * @property {'cmd'|'msbuild'|'devenv'} type
 * @property {PackageConditional=} condition
 * @property {string=} description
 * @property {PackageActionCmdOptions|PackageActionDevenvOptions|PackageActionDevEnvOptions} options
 */
...
class Packer extends EventEmitter {
    ...
    async _validateCondition(condition, variables) {
        ...
        if (typeof condition === 'string') {
            ...
            return !!(negate - (variables.hasOwnProperty(condition) && !!variables[condition]));
        }

        if (typeof condition === 'object' && condition) {
            const negate = condition.hasOwnProperty('negate') && !!condition['negate'];
            ...
            if (condition.hasOwnProperty('git_diff')) {
                let changes = await this._getGitChanges(condition['git_diff']['pattern']);
     ...
     */
    async _getGitChanges(/** string= */pattern, /** string= */sourceBase, /** boolean= */stripBase = false) {
        ...
        let changes = this.state.gitChanges;
    ...
    async _performPackage(rules, variables, outputFolder) {
        ...
        const addFileToArchive = function(source, dest, ignoreDuplicates = false) {
            ...
            await new Promise(async (resolve, reject) => {
                try {
                    ...
                    for (let rule of rules) {
                        ...
                        if (mode === 'git_diff') {
                            let changes = await this._getGitChanges(pattern, sourceBase, true);

                            if (typeof rule.exclude === 'string') {
                                const filter = minimatch.filter(rule.exclude, { nocase: true });
                                changes = changes.filter(x => {
                                    if (typeof x.path === 'string' && filter(x.path))
                                        return false;
                                    if (typeof x.toPath === 'string' && filter(x.toPath))
                                        return false;
                                    return true;
                                });
                            }
                            ...
                            if (mode === 'sync') {
                                ...
                                let unpackRule = unpackRules.find(x => x.type === 'sync' && x.path === destBase);
        ...
        await copyFileAsync(tmpName, zipPath);
        await unlinkAsync(tmpName);
        await writeFileAsync(cfgPath, JSON.stringify(unpackRules), { encoding: 'utf8' });
