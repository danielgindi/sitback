...
/**
 * @typedef {Object} PackageActionRemoveCommand
 * @property {string} path
 ...
 * @property {'cmd'|'msbuild'|'devenv'|'dotnet'|'rm'} type
 * @property {PackageConditional=} condition
 * @property {string=} description
 * @property {PackageActionCmdOptions|PackageActionMSBuildOptions|PackageActionDevenvOptions|PackageActionDotnetCommand|PackageActionRemoveCommand} options
 */

/**
 * @typedef {Object} PackageRuleDefinitionCommand
 * @property {string} path
 * @property {string[]?} args
 * @property {string?} cwd
 ...
 * @property {boolean?} match_dot
 * @property {PackageRuleDefinitionCommand|PackageRuleDefinitionCommand[]=} command
...
class Packer extends EventEmitter {
    ...
    async _validateCondition(condition, variables) {
        ...
        if (typeof condition === 'object' && condition) {
            ...
            if (hasOwnProperty.call(condition, 'git_diff')) {
                let changes = await this._getGitChanges(
                    condition['git_diff']['pattern'],
                    condition['git_diff']['exclude'],
                    condition['git_diff']['match_dot'],
                );
     ...
     */
    async _getGitChanges(/** string? */pattern, /** string|string[]? */exclude, /** boolean? */dot, /** string? */sourceBase, /** boolean? */stripBase = false) {
        ...
        if (pattern !== undefined) {
            ...
            const filter = minimatch.filter(pattern, { nocase: true, dot: !!dot });
        ...
        if (typeof exclude === 'string' || (Array.isArray(exclude) && exclude.length)) {
            const filters = [].concat(exclude)
                ...
                .map(x => minimatch.filter(x, { nocase: true, dot: !!dot }));
    ...
    async _performActions(actions, variables) {
        for (let action of actions) {
            ...
            switch (action.type) {
                ...
                case 'rm': {
                    if (Fs.rm) {
                        await promisify(Fs.rm)(options.path, {
                        ...
                        // if the path is a file, call unlink. otherwise call rmdir
                        let stats = await Fs.promises.stat(options.path);
                        if (stats.isFile()) {
                            await promisify(Fs.unlink)(options.path);
                        } else {
                            await promisify(Fs.rmdir)(options.path, {
                                maxRetries: options.maxRetries ?? 0,
                                retryDelay: options.retryDelay ?? 100,
                                recursive: options.recursive ?? false,
                            });
                ...
                for (let rule of rules) {
                    ...
                    switch (mode) {
                        case 'git_diff': {
                            ...
                            let changes = await this._getGitChanges(pattern, excludePack, rule.match_dot, sourceBase, true);
                        ...
                        case 'partial_sync': {
                            ...
                            const filter = minimatch.filter(pattern, { nocase: true, dot: !!rule.match_dot });

                            let files = [];
                            try {
                                const stats = await Fs.promises.stat(globRoot);
                                if (stats.isDirectory())
                                    files = await recursiveReaddir(globRoot);
                            } catch (err) {
                                // ignore, directory does not exist
                            }

                            files = files.map(x => x.substring(globRoot.length));
                            let matches = files.filter(filter);

                            let excludePack = (rule.excludeInPackage ?? []).concat(rule.exclude ?? []);

                            if (typeof excludePack === 'string' || (Array.isArray(excludePack) && excludePack.length)) {
                                const filters = [].concat(excludePack)
                                    .map(x => x.startsWith('/') ? x : `/${x}`)
                                    .map(x => minimatch.filter(x, { nocase: true, dot: !!rule.match_dot }));
                                const filter = (path) => filters.some(f => f(path));
